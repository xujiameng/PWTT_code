...
% [heartRate,peak]  = detectionRR2(SF,fs);%对  归一化  的ECG求R波峰值点
...
%     if size(cache,1)>1
%         num=0;
%         for j=1:size(cache,1)-1
%             if cache(j+1,2)-cache(j,2)==1
%                 num=num+1;
%                if o_as(cache(j+1,1)) > o_as(cache(j,1))
%                    zhiling(num)=j+1;
%                else
%                    zhiling(num)=j;
%                end
%                for h=1:length(zhiling)
%                cache(zhiling(h),1)=0;
%                end
%             end
%         end
